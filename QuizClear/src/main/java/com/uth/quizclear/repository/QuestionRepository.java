...
public interface QuestionRepository extends JpaRepository<Question, Long>, JpaSpecificationExecutor<Question> {
    ...
    List<Question> findQuestionsWithSimilarContent(@Param("excludeId") Long excludeId,
    ...
    // Additional method for lecturer scope
    long countByCreatedBy(com.uth.quizclear.model.entity.User createdBy);
    ...
    // Note: No direct relation between Course and Subject, so we query by subject
    // code/name in course department
    ...
    List<Question> findByDepartmentScopeAndStatus(@Param("departmentName") String departmentName,
    ...
    List<Question> findBySubjectScopeAndStatus(@Param("subjectName") String subjectName,
            @Param("status") QuestionStatus status);
    // Note: Removed duplicate methods findQuestionsAccessibleByDepartment and
    // findQuestionsAccessibleBySubject
    ...
    List<Question> findQuestionsForDepartmentDuplicateCheck(@Param("departmentName") String departmentName,
    ...
    List<Question> findQuestionsForSubjectDuplicateCheck(@Param("subjectName") String subjectName,
            @Param("excludeId") Long excludeId);
    ...
    List<Question> findByDepartmentScopeAndDifficulty(@Param("departmentName") String departmentName,
    ...
    List<Question> findBySubjectScopeAndDifficulty(@Param("subjectName") String subjectName,
            @Param("difficulty") DifficultyLevel difficulty);

    long countByTaskIdAndStatus(Integer taskId, QuestionStatus status);

    // Count questions by planId
    @Query("SELECT COUNT(q) FROM Question q WHERE q.planId = :planId")
    long countByPlanId(@Param("planId") Long planId);

    // Count questions by planId and status
    @Query("SELECT COUNT(q) FROM Question q WHERE q.planId = :planId AND q.status = :status")
    long countByPlanIdAndStatus(@Param("planId") Long planId, @Param("status") QuestionStatus status);

    // Count approved questions by planId, status and approved_by (for HoD progress
    // tracking)
    @Query("SELECT COUNT(q) FROM Question q WHERE q.planId = :planId AND q.status = :status AND q.approver.userId = :approvedBy")
    long countByPlanIdAndStatusAndApprovedBy(@Param("planId") Long planId, @Param("status") QuestionStatus status,
    ...
    List<Question> findQuestionsPendingReviewByDepartment(@Param("department") String department); // Find users who
                                                                                                   // have
                                                                                                   // created questions
                                                                                                   // in
                                                                                                   // a specific
